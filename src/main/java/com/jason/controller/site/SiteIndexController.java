...
public class SiteIndexController {
    ...
    @GetMapping("/friends")
    public String friends(ParamDTO paramDTO,
                        Model model,
...
//        model.addAttribute("articleList",articleList) ;
        ...
        PageInfo<Friends> friends = friendsService.getAllFriends(1, 100);
        ...
        if(request.getHeader("X-Pjax") != null){
            return "site/common-template/friends" ;
        }
        return "site/friends" ;
    }

    @GetMapping(path = {"/search"})
    public String articleSearch(String title, ParamDTO paramDTO,
                        Model model,
                        HttpServletRequest request){
        ArticleGetDTO articleGetDTO = new ArticleGetDTO() ;
        articleGetDTO.setPublished(ArticlePublish.PUBLISH);
        articleGetDTO.setTitle(title);
        PageInfo<Article> articleList =
                articleService.getArticles(
                        articleGetDTO,
                        paramDTO.getArticlePage(),
